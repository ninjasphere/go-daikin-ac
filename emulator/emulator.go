...
func (d *emulatedWirelessAC) handleUDP(src *net.UDPAddr, n int, b []byte) {
	log.Println(n, "udp bytes read from", src)
	log.Println(hex.Dump(b[:n]))

	c, err := net.DialUDP("udp", nil, src)
	spew.Dump("Dialed back to app", err)
	i, err := c.Write([]byte("ret=OK,type=aircon,reg=th,dst=1,ver=2_2_5,pow=0,err=0,location=0,name=Ninja%20Blocks%20Wuz%20Here,icon=1,method=polling,port=30050,id=000005zr,pw=pyrbtetn,lpw_flag=0,adp_kind=2,pv=0,cpv=0,led=0,en_setzone=1,mac=FCC2DE460A5C,adp_mode=run\n"))
